...
func Test_jsonpath_JsonPathLookup_1(t *testing.T) {
	...
	// multiple index
	res, err = Lookup(json_data, "$.store.book[0,1].title")
	t.Log(err, res)
	...
	// full array
	res, err = Lookup(json_data, "$.store.book[0:].price")
	t.Log(err, res)
	...
	// complex
	res, err = Lookup(json_data, "$.store.book[?(@.price < $.expensive)].price")
	t.Log(err, res)
	if res_v, ok := res.([]interface{}); ok != true || res_v[0].(float64) != 8.95 || res_v[1].(float64) != 8.99 {
		t.Errorf("exp: [8.95, 8.99], got: %v", res)
	}
}

func Test_jsonpath_JsonPathReplace_1(t *testing.T) {
	var json_data interface{}
	json.Unmarshal(jsdata, &json_data)
	// key from root
	Replace(json_data, "$.expensive", float64(20))
	res, _ := Lookup(json_data, "$.expensive")
	if res_v, ok := res.(float64); ok != true || res_v != 20 {
		t.Errorf("expensive should be 20")
	}

	// single index
	Replace(json_data, "$.store.book[0].price", float64(1.2))
	res, _ = Lookup(json_data, "$.store.book[0].price")
	if res_v, ok := res.(float64); ok != true || res_v != 1.2 {
		t.Errorf("$.store.book[0].price should be 1.2")
	}

	// nagtive single index
	Replace(json_data, "$.store.book[-1].isbn", "test")
	res, _ = Lookup(json_data, "$.store.book[-1].isbn")
	if res_v, ok := res.(string); ok != true || res_v != "test" {
		t.Errorf("$.store.book[-1].isbn should be \"test\"")
	}

	// multiple index
	Replace(json_data, "$.store.book[0,1].price", float64(1.2))
	res, err := Lookup(json_data, "$.store.book[0,1].price")
	t.Log(err, res)
	if res_v, ok := res.([]interface{}); ok != true || res_v[0].(float64) != 1.2 || res_v[1].(float64) != 1.2 {
		t.Errorf("exp: [1.2, 1.2], got: %v", res)
	}

	// multiple index
	Replace(json_data, "$.store.book[0,1].title", "title_test")
	res, err = Lookup(json_data, "$.store.book[0,1].title")
	t.Log(err, res)
	if res_v, ok := res.([]interface{}); ok != true {
		if res_v[0].(string) != "title_test" || res_v[1].(string) != "title_test" {
			t.Errorf("title are wrong: %v", res)
		}
	}
	...
	res, err = Lookup(json_data, "$.store.book[0:].price")
	t.Log(err, res)
	if res_v, ok := res.([]interface{}); ok != true || res_v[0].(float64) != 1.2 || res_v[1].(float64) != 1.2 || res_v[2].(float64) != 1.2 || res_v[3].(float64) != 1.2 {
		t.Errorf("exp: [1.2,1.2, 1.2, 1.2], got: %v", res)
	}

	// range
	Replace(json_data, "$.store.book[0:1].price", float64(1.3))
	res, err = Lookup(json_data, "$.store.book[0:1].price")
	t.Log(err, res)
	if res_v, ok := res.([]interface{}); ok != true || res_v[0].(float64) != 1.3 || res_v[1].(float64) != 1.3 {
		t.Errorf("exp: [1.3, 1.3], got: %v", res)
	}
...
func Test_jsonpath_JsonPathLookup_filter(t *testing.T) {
	res, err := Lookup(json_data, "$.store.book[?(@.isbn)].isbn")
	t.Log(err, res)
	...
	res, err = Lookup(json_data, "$.store.book[?(@.price > 10)].title")
	t.Log(err, res)
	if res_v, ok := res.([]interface{}); ok != true {
	...
	res, err = Lookup(json_data, "$.store.book[?(@.price > 10)]")
	t.Log(err, res)
...
func Test_jsonpath_parse_token(t *testing.T) {
	for idx, tcase := range parse_token_cases {
		...
		step, err := parse_token(token)
		t.Logf("[%d] - expected: op: %v, key: %v, args: %v\n", idx, exp_op, exp_key, exp_args)
		t.Logf("[%d] - got: err: %v, op: %v, key: %v, args: %v\n", idx, err, step.op, step.key, step.args)
		if step.op != exp_op {
			t.Errorf("ERROR: op(%v) != exp_op(%v)", step.op, exp_op)
			return
		}
		if step.key != exp_key {
			t.Errorf("ERROR: key(%v) != exp_key(%v)", step.key, exp_key)
			return
		}

		if step.op == "idx" {
			if args_v, ok := step.args.([]int); ok == true {
				...
				t.Errorf("ERROR: idx op should expect args:[]int{} in return, (got)%v", reflect.TypeOf(step.args))
				return
			}
		}

		if step.op == "range" {
			if args_v, ok := step.args.([2]interface{}); ok == true {
				fmt.Println(args_v)
				...
				t.Errorf("ERROR: range op should expect args:[2]interface{}, (got)%v", reflect.TypeOf(step.args))
				return
			}
		}

		if step.op == "filter" {
			if args_v, ok := step.args.(string); ok == true {
				fmt.Println(args_v)
				...
				t.Errorf("ERROR: filter op should expect args:[]string{}, (got)%v", reflect.TypeOf(step.args))
			}
		}
	}
}
...
func Test_jsonpath_get_key(t *testing.T) {
	...
	res, err := emptyStep.get_key(obj, "key")
	fmt.Println(err, res)
	if err != nil {
	...
	res, err = emptyStep.get_key(obj, "hah")
	fmt.Println(err, res)
	if res != nil {
	...
	obj3 := map[string]interface{}{"key": "hah"}
	res, err = emptyStep.get_key(obj3, "key")
	...
	res, err = emptyStep.get_key(obj4, "a")
	fmt.Println(err, res)
}

func Test_jsonpath_get_idx(t *testing.T) {
	obj := []interface{}{1, 2, 3, 4}
	res, err := emptyStep.get_idx(obj, 0)
	fmt.Println(err, res)
	if err != nil {
	...
	res, err = emptyStep.get_idx(obj, 2)
	fmt.Println(err, res)
	...
	res, err = emptyStep.get_idx(obj, -1)
	fmt.Println(err, res)
	if err != nil {
	...
	res, err = emptyStep.get_idx(obj, -5)
	fmt.Println(err, res)
	if err == nil {
		t.Errorf("index out of range  error not raised")
		return
	...
	obj2 := []interface{}{1, 2, 3, 4}
	res, err = emptyStep.get_idx(obj2, 0)
	fmt.Println(err, res)
	if err != nil {
...
func Test_jsonpath_get_range(t *testing.T) {
	...
	res, err := emptyStep.get_range(obj, 0, 2)
	fmt.Println(err, res)
	if err != nil {
	...
	res, err = emptyStep.get_range(obj1, 3, -1)
	fmt.Println(err, res)
	if err != nil {
		t.Errorf("failed to get_range: %v", err)
	}
	...
	obj2 := 2
	res, err = emptyStep.get_range(obj2, 0, 1)
	fmt.Println(err, res)
	if err == nil {
...
func Test_jsonpath_parse_filter(t *testing.T) {
	...
	for _, tcase := range tcase_parse_filter {
		f, _ := parse_filter(tcase["filter"].(string))
		t.Log(tcase)
		t.Logf("lp: %v, op: %v, rp: %v", f.lp, f.op, f.rp)
		if f.lp != tcase["exp_lp"].(string) {
			t.Errorf("%s(got) != %v(exp_lp)", f.lp, tcase["exp_lp"])
			return
		}
		if f.op != tcase["exp_op"].(string) {
			t.Errorf("%s(got) != %v(exp_op)", f.op, tcase["exp_op"])
			return
		}
		if f.rp != tcase["exp_rp"].(string) {
			t.Errorf("%s(got) != %v(exp_rp)", f.rp, tcase["exp_rp"])
			return
		}
	}
}
...
func Test_jsonpath_eval_filter(t *testing.T) {
	for idx, tcase := range tcase_eval_filter[1:] {
		...
		exp := tcase["exp"].(bool)
		t.Logf("idx: %v, lp: %v, op: %v, rp: %v, exp: %v", idx, f.lp, f.op, f.rp, exp)
		got, err := f.eval_filter(obj, root)
...
func Test_jsonpath_cmp_any(t *testing.T) {
	for idx, tcase := range tcase_cmp_any {
		...
		res, err := f.cmp_any(tcase["obj1"], tcase["obj2"])
		exp := tcase["exp"].(bool)
		exp_err := tcase["err"]
		if exp_err != nil {
			if err == nil {
...
func Test_jsonpath_string_equal(t *testing.T) {
	...
	res, err := Lookup(j, "$.store.book[?(@.author == 'Nigel Rees')].price")
	t.Log(res, err)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
`
	...
	res, err := Lookup(j, "$.head_commit.author.username")
	t.Log(res, err)
}
...
func Test_jsonpath_num_cmp(t *testing.T) {
	...
	var j interface{}
	json.Unmarshal([]byte(data), &j)
	res, err := Lookup(j, "$.books[?(@.price > 100)].name")
	if err != nil {
		t.Fatal(err)
	}
	arr := res.([]interface{})
...
func BenchmarkJsonPathLookup_0(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Lookup(json_data, "$.expensive")
	}
}

func BenchmarkJsonPathLookup_1(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Lookup(json_data, "$.store.book[0].price")
	}
}

func BenchmarkJsonPathLookup_2(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Lookup(json_data, "$.store.book[-1].price")
	}
}

func BenchmarkJsonPathLookup_3(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Lookup(json_data, "$.store.book[0,1].price")
	}
}

func BenchmarkJsonPathLookup_4(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Lookup(json_data, "$.store.book[0:2].price")
	}
}

func BenchmarkJsonPathLookup_5(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Lookup(json_data, "$.store.book[?(@.isbn)].price")
	}
}

func BenchmarkJsonPathLookup_6(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Lookup(json_data, "$.store.book[?(@.price > 10)].title")
	}
}

func BenchmarkJsonPathLookup_7(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Lookup(json_data, "$.store.book[?(@.price < $.expensive)].price")
	}
}

func BenchmarkJsonPathLookup_8(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Lookup(json_data, "$.store.book[:].price")
	}
}

func BenchmarkJsonPathLookup_9(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Lookup(json_data, "$.store.book[?(@.author == 'Nigel Rees')].price")
	}
}

func BenchmarkJsonPathLookup_10(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Lookup(json_data, "$.store.book[?(@.author =~ /(?i).*REES/)].price")
	}
}
...
func TestReg(t *testing.T) {
	...
	res, err := Lookup(json_data, "$.store.book[?(@.author =~ /(?i).*REES/i)].author")
	t.Log(err, res)
...
func TestRegReplace(t *testing.T) {
	var json_data interface{}
	err := json.Unmarshal(jsdata, &json_data)
	t.Log(err)
	err = Replace(json_data, "$.store.book[?(@.author =~ /(?i).*REES/i)].price", 1.23)
	t.Log(err)
	res, err := Lookup(json_data, "$.store.book[?(@.author =~ /(?i).*REES/i)].price")
	t.Log(err, res)
	author := res.([]interface{})[0].(float64)
	t.Log(author)
	if author != 1.23 {
		t.Fatal("should be `1.23` but got: ", author)
	}
}
